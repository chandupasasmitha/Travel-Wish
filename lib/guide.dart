...
// import 'guide_details_page.dart'; // Uncomment when you create the details page
...
class _GuideState extends State<Guide> {
  ...
  Future<void> fetchGuides() async {
    ...
    try {
      final data = await Api.getGuides();
      setState(() {
        guides = data;
        isLoading = false;
      });
  ...
  List<Map<String, dynamic>> getSortedAndFilteredGuides() {
    List<Map<String, dynamic>> filtered = guides;

    // Apply location filter
    if (selectedLocation != 'all') {
      filtered = filtered.where((guide) {
        List<dynamic> locations = guide['coveredLocations'] ?? [];
    ...
    if (selectedLanguage != 'all') {
      filtered = filtered.where((guide) {
        List<dynamic> languages = guide['languages'] ?? [];
  ...
  Widget build(BuildContext context) {
    final displayedGuides = getSortedAndFilteredGuides();
                    ...
                    child: displayedGuides.isEmpty
                        ...
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              final guide = displayedGuides[index];
                              return GuideCard(
                                guide: guide,
                                onTap: () {
                                  // Navigate to guide details page
                                  // Uncomment when you create the guide_details_page.dart
                                  /*
                                  Navigator.push(
                                    context,
                                    MaterialPageRoute(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      Wrap(
                        spacing: 4,
                        runSpacing: 4,
                        children: _GuideState._buildLanguageChips(languages),
                      ),
                      SizedBox(height: 8),
                      Row(
                        children: [
